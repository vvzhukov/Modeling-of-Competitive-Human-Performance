...
datasets <- c("SCHOLARS_CS", "SCHOLARS_BIO")

tmpSCL<-list(read.csv(paste(data_path,"/temporal_differential/Scholars_CS_differential.csv", sep=""),header=T,sep=","),
               read.csv(paste(data_path,"/temporal_differential/Scholars_BIO_differential.csv", sep=""),header=T,sep=","))

VarNamesL<-list(c("NSF_1998", "NSF_2001", "NSF_2004", "NSF_2007", "NSF_2010", "NSF_2013", "NSF_2016"),
                c("NIH_1998", "NIH_2001", "NIH_2004", "NIH_2007", "NIH_2010", "NIH_2013", "NIH_2016"))
...
SC_time<-c(3,6,9,12,15,18,21)		### 3=1998, 6=2001, 9=2004, 12=2007, 15=2010, 18=2013, 21=2016


SC_1998<-tmpSC[tmpSC[,3]==CTV[SC_time[1]],2]
SC_2001<-tmpSC[tmpSC[,3]==CTV[SC_time[2]],2]
SC_2004<-tmpSC[tmpSC[,3]==CTV[SC_time[3]],2]
SC_2007<-tmpSC[tmpSC[,3]==CTV[SC_time[4]],2]
SC_2010<-tmpSC[tmpSC[,3]==CTV[SC_time[5]],2]
SC_2013<-tmpSC[tmpSC[,3]==CTV[SC_time[6]],2]
SC_2016<-tmpSC[tmpSC[,3]==CTV[SC_time[7]],2]

Data<-list(ns(SC_1998), ns(SC_2001), ns(SC_2004), ns(SC_2007), ns(SC_2010), ns(SC_2013), ns(SC_2016))

n<-length(VarNames)			### This is the number of the recorded variables (i.e. columns in the list Data)


setwd(paste(data_path, "/scripts_results/DIFFERENTIAL/", datasets[i], sep=""))
...
### col1=gof, col2=xmin, col3=mean, col4=sigma, col5=ntail and layers refer to the different data sets
tic<-date()
k <- 0 ### Counter for the data update, seed randomizer
...
main_function=function(x){
    tryCatch(
        ...
        error=function(cond) {
            ...
            x <<- x + sample(c(0,1), replace=TRUE, size=length(x))
            ### add random vector with (0,1) values to the dataset
            
            k <<- k + 1
            
            print(paste("Error caught! Error text: ", cond))
            main_function(x) } ### Recursive call    

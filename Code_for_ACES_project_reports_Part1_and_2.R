...
### (NOT the temporal evolution)and it contains the Descriptive
### statistics, non-parametric approach for Total data and the
### parametric modeling for the total complete data, i.e. material 
### for Part 1 and Part 2 of the reports.
...
### Splitting GBR in halves:
OMS_GBR <- OMS_GBR[seq(1, length(OMS_GBR), 2)]
...
### Here we will put the histograms and optionally with the overlaid density plots
x11()		### when we run in PC uncomment this
#quartz()	###  when we run in MAC uncomment this
par(mfrow=c(3,3), mar=c(1,1,1,1),oma=c(1,1,1,0))
par(mgp=c(0,1,0))
for (i in 1:n){
	if (i==6){
...
title("Histograms of the original data sets",outer=T,cex.main=2,col.main="blue",line=-0.5)


### Summary stats: where rows are data sets and cols are 
### sample size, Min, Q1, Median, Q3, Max, Mean, SD, Skewness
SSall<-matrix(NA,n, 9)	
for (i in 1:n){
	Y<-Data[[i]]
...
### Table with WW2 pilot data
### we report the frequencies where rows are countries and columns 
### refer to counts (4,5,6,7,8,>8)
TableWW2<-matrix(NA, 3,6)	
for (i in 1:3){
	Y<-Data[[i]]
	tmpc<-1
	for (j in 4:8){
		TableWW2[i,tmpc]<-sum(Y==j)/length(Y)*100
		tmpc<-tmpc+1
	}
	TableWW2[i,tmpc]<-sum(Y>8)/length(Y)*100
}
round(TableWW2,2)


### Table with Schoolar Grant data
TableSG<-matrix(NA, 2,6)		### we report the frequencies where rows are countries and columns refer to counts (1,2,3,4,5,>5)
for (i in 4:5){
	Y<-Data[[i]]
	tmpc<-1
	for (j in 1:5){
		TableSG[i-3,tmpc]<-sum(Y==j)/length(Y)*100
		tmpc<-tmpc+1
	}
	TableSG[i-3,tmpc]<-sum(Y>5)/length(Y)*100
}
round(TableSG,2)


### Table with Olympic Medals Score data
TableOMS<-matrix(NA, 3, 4)		### we report the frequencies where rows are countries and columns refer to counts (1,2,3,>4)
for (i in 6:8){
	Y<-Data[[i]]
	tmpc<-1
	for (j in 1:3){
		TableOMS[i-5,tmpc]<-sum(Y==j)/length(Y)*100
		tmpc<-tmpc+1
	}
	TableOMS[i-5,tmpc]<-sum(Y>3)/length(Y)*100
}
round(TableOMS,2)
...
### Summary stats: where rows are data sets and cols are 
### (sample size, Min, Q1, Median, Q3, Max, Mean, SD, Skewness)
SSallTR<-matrix(NA,n, 9)		
for (i in 1:n){
	Y<-DataTR[[i]]
	SSallTR[i,1]<-length(Y)			## sample size
	SSallTR[i,2:6]<-quantile(Y,c(0,0.25,0.5,0.75,1))		### Min, Q1, Median, Q3, Max
	SSallTR[i,7]<-mean(Y)				### mean of response variable
	SSallTR[i,8]<-	sd(Y)				### sd of response variable
	SSallTR[i,9]<-	skewness(Y)	### skewness of response variable
}

round(SSallTR,3)


### Here we will put the histograms along with optional the 
# overlaid density plots for the truncated data
x11()
#quartz()
par(mfrow=c(3,3), mar=c(1,1,1,1),oma=c(1,1,1,0))
par(mgp=c(0,1,0))
for (i in 1:n){
	...
	tmpData<-DataTR[[i]]
	hist(tmpData,freq=F,xlab="",ylab="",main="")
	ifelse(i<4,tmpSP<-6,ifelse(i>5,tmpSP<-4,tmpSP<-2))
	###lines(density(tmpData,from=tmpSP),col="blue",lwd=1)
	legend("top",legend=paste(VarNames[i]), cex=2, bty="n")
}
title("Histograms of the truncated data sets",outer=T,cex.main=2,col.main="blue",line=-0.5)
...
######################################################################
### HERE WE WILL PERFORM THE NON-PARAMETRIC APPROACH TO EXAMINE 
### DISTRIBUTIONAL AGREEMENT AMONG ALL PAIRS OF DATA WITH QQ-PLOTS
######################################################################
...
x11()
#quartz()
par(mfcol=c(n,n), mar=c(1,1,1,1),oma=c(1,1,1,1))
par(mgp=c(0,1,0))
for (i in 1:n){
	for (j in 1:n){
		if (i==j){	### Here we will plot the Histogram and the density curve over histogram
			tmpData<-Data[[i]]
			hist(tmpData,freq=F,xlab="",ylab="",main="")
			legend("topright",paste(VarNames[i]),cex=1.5,bty="n")
			lines(density(tmpData),col="blue",lwd=2)			
		}
		if (i<j){	### Here we will plot the qq-plot and the regression line
			tmpDataY<-Data[[i]]
			tmpDataX<-Data[[j]]
			tmpplot<-qqplot(tmpDataX,tmpDataY,xlab="",ylab="")
			#abline(lm(tmpplot$y~tmpplot$x),col="red",lwd=2)
		}
		if (i>j){	### Here we will print the correlation coefficient of the qq-plot
			tmpDataY<-Data[[i]]
			tmpDataX<-Data[[j]]
...
title("QQ-plots, histograms and density estimates for the original data",outer=T,cex.main=2,col.main="blue",line=-0.5)
...
title("QQ-plots, histograms and density estimates for the original data (red line is fitted OLS r is the correlation)",outer=T,cex.main=2,col.main="blue",line=-0.5)


### qq-plots for the TRUNCATED data
...
for (i in 1:n){
	for (j in 1:n){
		if (i==j){	### Here we will plot the Histogram and the density curve over histogram
			tmpData<-DataTR[[i]]
			hist(tmpData,freq=F,xlab="",ylab="",main="")
			legend("topright",paste(VarNamesTR[i]),cex=1.3,bty="n")			
			ifelse(i<4,tmpSP<-6,ifelse(i>5,tmpSP<-4,tmpSP<-2))
			lines(density(tmpData,from=tmpSP),col="blue",lwd=2)
		}
		if (i<j){	### Here we will plot the qq-plot and the regression line
			tmpDataY<-DataTR[[i]]
			tmpDataX<-DataTR[[j]]
...
title("QQ-plots, histograms and density estimates for the truncated data",outer=T,cex.main=2,col.main="blue",line=-0.5)
...
title("QQ-plots, histograms and density estimates for the truncated data (red line is fitted OLS r is the correlation)",outer=T,cex.main=2,col.main="blue",line=-0.5)
...
for (i in n:1){
  try({
	...
	if (i==8){
	memory.limit()	### Clean the memory as we get out of memory otherwise in the simulations																### col1=gof, col2=xmin, col3=mean, col4=sigma, col5=ntail and layers refer to the different data sets
	}
...
par(mfcol=c(1,1), mar=c(5,4,2,0),oma=c(1,1,1,1))
par(mgp=c(2.5,1,0))
plot(PLest[,7],type="b",col="coral",pch=15,lty=1,ylim=c(0,max(PLest[,7],LNest[,7])), xaxt="n", xlab="", ylab="p-values", cex.lab=1.4, lwd=2, main="P-values of the fitted model")

...
#filtering swimmers
athletes_US %>%
    filter(str_detect(Sport, 'Swimming')) -> athletes_US


# up to 1960 and from 1964 till today

# total
gen1 <- athletes_US[,c(6:11)]
gen2 <- athletes_US[,c(12:20)]
gen3 <- athletes_US[,c(21:36)]
gen4 <- athletes_US[,c(37:56)]


US_total_sw <- data.frame(rowSums(gen1), rowSums(gen2), rowSums(gen3), rowSums(gen4), stringsAsFactors = FALSE)

colnames(US_total_sw) <- c("1896-1912", "1920-1936", "1948-1976", "1980-2016")
...
ns <- function (data){
    data2 <- head(sort(na.omit(data[data>0]), decreasing = TRUE), 1600) #Limit for entries to deal with out-of-memory errors
    return(data2)
}

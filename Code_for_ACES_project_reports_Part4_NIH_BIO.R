...
### i.e. chronological evaluation of the NIH BIO grants
...
#setwd("/Users/pt/CPL/Vitalii/Data/Data_used_in_analysis/Data_chonological_evolution")
setwd("D:/Vitalii/Data")		### This is for the Office PC

### Here we will load the NSF and NIH grants obtained in chronological order
tmpNSF<-read.csv("./Scholars/NSF_CS_scholars_QC_temporal_v2.csv",header=T,sep=",")
tmpNIH<-read.csv("./Scholars/NIH_BIO_scholars_QC_temporal_v2.csv",header=T,sep=",")

### Initially we plot the data NSF and NIH as they evelve per time
NSFall<-as.matrix(tmpNSF,dim(tmpNSF)[1],dim(tmpNSF)[1])
NSFall<-NSFall[,-1]
NSFallnz<-NSFall
NSFallnz[NSFallnz==0]<-NA

NIHall<-as.matrix(tmpNIH,dim(tmpNIH)[1],dim(tmpNIH)[1])
NIHall<-NIHall[,-1]
NIHallnz<-NIHall
NIHallnz[NIHallnz==0]<-NA



### Here we find all different profiles of scholars
#UNSFall<-unique(NSFall)
#WUNSFall<-rep(NA,dim(UNSFall)[1])
#for (i in 1:dim(UNSFall)[1]){
#	tmpc<-0
#	for (j in 1:dim(NSFallnz)[1]){
#		tmpc<-tmpc + (sum(abs(NSFall[j,]-UNSFall[i,]))==0)*1
#		}
#	WUNSFall[i]<-tmpc
#}
#UNIHall<-unique(NIHall)
#WUNIHall<-rep(NA,dim(UNIHall)[1])
#for (i in 1:dim(UNIHall)[1]){
#	tmpc<-0
#	for (j in 1:dim(NIHallnz)[1]){
#		tmpc<-tmpc + (sum(abs(NIHall[j,]-UNIHall[i,]))==0)*1
#		}
#	WUNIHall[i]<-tmpc
#}


### Periods which we will split the phenomenon to study it.
### [1996,1998], [1999,2001], [2002,2004], [2005,2007], [2008,2010], [2011,2013], [2014,2016]

### Times at which we will study the evolution:
G_time<-seq(3,21,by=3)		### 3=1998, 6=2001, 9=2004, 12=2007, 15=2010, 18=2013, 21=2016


NIH_1998<-sort(NIHallnz[,G_time[1]])	### NIH grants until 1998
NIH_2001<-sort(NIHallnz[,G_time[2]])	### NIH grants until 2001
NIH_2004<-sort(NIHallnz[,G_time[3]])	### NIH grants until 2004
NIH_2007<-sort(NIHallnz[,G_time[4]])	### NIH grants until 2007
NIH_2010<-sort(NIHallnz[,G_time[5]])	### NIH grants until 2010
NIH_2013<-sort(NIHallnz[,G_time[6]])	### NIH grants until 2013
NIH_2016<-sort(NIHallnz[,G_time[7]])	### NIH grants until 2016

Data<-list(NIH_1998, NIH_2001, NIH_2004, NIH_2007, NIH_2010, NIH_2013, NIH_2016)
VarNames<-c("NIH_1998", "NIH_2001", "NIH_2004", "NIH_2007", "NIH_2010", "NIH_2013", "NIH_2016")
...
#pdf(file="C:\\Users\\PT_office\\Desktop\\Aces_project\\Plots_evolution\\NIH_chronological_evolution.pdf")

### Time series of the NSF (blue) and NIH (pink) per year (1996-2016)
xlim1<-c(1,21)
ylim1<-c(min(NSFall,NIHall),max(NSFall,NIHall))
x11()
par(mfrow=c(2,2), mar=c(2,1,3,2),oma=c(1,1,1,0))
par(mgp=c(0,0.4,0))
plot(0, 0, type="n", xlim=xlim1, ylim=ylim1,xaxt="n",xlab="",ylab="",main=paste("NSF: # of grants, n = ",dim(NSFallnz)[1]))
axis(1, at=1:21,labels=1996:2016,las=2)
for (i in 1:(dim(NSFallnz)[1])){
	lines(NSFallnz[i,],col="blue")
}
plot(0, 0, type="n", xlim=xlim1, ylim=ylim1,xaxt="n",xlab="",ylab="",main=paste("NIH: # of grants, n = ",dim(NIHallnz)[1]))
axis(1, at=1:21,labels=1996:2016,las=2)
for (i in 1:(dim(NIHallnz)[1])){
	lines(NIHallnz[i,],col="pink")
}
plot(0, 0, type="n", xlim=xlim1, ylim=c(0,1),xaxt="n",xlab="",ylab="",main=paste("NSF: % grant progress, n = ",dim(NSFallnz)[1]))
axis(1, at=1:21,labels=1996:2016,las=2)
for (i in 1:(dim(NSFall)[1])){
	lines(NSFallnz[i,]/NSFallnz[i,21],col="blue")
}
plot(0, 0, type="n", xlim=xlim1, ylim=c(0,1),xaxt="n",xlab="",ylab="",main=paste("NIH: % grant progress, n = ",dim(NIHallnz)[1]))
axis(1, at=1:21,labels=1996:2016,las=2)
for (i in 1:(dim(NIHall)[1])){
	lines(NIHallnz[i,]/NIHallnz[i,21],col="pink")
}
title("NSF (blue) and NIH (pink) grant evolution",outer=T,cex.main=2,col.main="black",line=-0.5)
...
title("Histograms of the NIH Grants data sets",outer=T,cex.main=2,col.main="blue",line=-0.5)

...
# 1948 - 1960
athletes_US_sw$started_at2 <- "none"

athletes_US_sw$started_at2 <- ifelse(rowSums(athletes_US_sw[,c(21:28)]) != 0,
                                     "required", 
                                     athletes_US_sw$started_at2)


gen1 <- rowSums(subset(athletes_US_sw, started_at2 == "required")[,c(6:56)])


US_total_sw2 <- data.frame(gen1, stringsAsFactors = FALSE)
colnames(US_total_sw2) <- c("1948-1960")


datasets <- c(
              "ATHLETES_USA_SWIMMERS_WW2_GR"
              )

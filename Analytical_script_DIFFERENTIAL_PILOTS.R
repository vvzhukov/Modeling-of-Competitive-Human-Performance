...
datasets <- c(#"PILOTS_MEDIAN", 
              "PILOTS_PHASES") 
              #"PILOTS_YEAR")

tmpSCL<-list(#read.csv(paste(data_path,"/temporal_cumulative/Pilots_cumulative_median.csv", sep=""),header=T,sep=","),
               read.csv(paste(data_path,"/temporal_differential/Pilots_differential_phases.csv", sep=""),header=T,sep=","))
                #read.csv(paste(data_path,"/temporal_cumulative/Pilots_cumulative_year.csv", sep=""),header=T,sep=","))

VarNamesL<-list(#c("1939h1", "1939h2", "1940h1", "1940h2", "1941h1", "1941h2", "1942h1", "1942h2", "1943h1", "1943h2", "1944h1", "1944h2", "1945h1", "1945h2"),
                c("phase1", "phase2", "phase3", "phase4"))
                #c("1939", "1940", "1941", "1942", "1943", "1944", "1945"))
...
{
    Data <- lappend(Data, ns(tmpSC[tmpSC[,3]==CTV[k],2]))
}
...
main_function=function(x){
    ...
    #### ISSUES with x = c(1, 1, 1, 2, 2, 1, 4, 1, 1), works with c(1, 1, 1, 3, 2, 1, 4, 1, 1)
    m_ln <<- dislnorm$new(x)

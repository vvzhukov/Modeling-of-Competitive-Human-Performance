...
### data_path <- "D:/Vitalii/Data"		### This is for the Office PC
...
tmpWW2_USA<-read.csv(paste(data_path,"/Total/Pilots_total_USA.csv", sep=''),header=T,sep=",")
WW2_USA<-tmpWW2_USA[,2]

### Here we will load the WW2 GERMAN pilot data 
tmpWW2_GER<-read.csv(paste(data_path,"/Total/Pilots_total_GER.csv", sep=''),header=T,sep=",")
WW2_GER<-tmpWW2_GER[,2]

### Here we will load the WW2 GBR pilot data 
tmpWW2_RAF<-read.csv(paste(data_path,"/Total/Pilots_total_GBR.csv", sep=''),header=T,sep=",")
WW2_RAF<-tmpWW2_RAF[,2]

## SCHOLARS

### Here we will load the NSF CS scholar data 
tmpNSF_CS<-read.csv(paste(data_path,"/Total/Scholars_total_CS.csv", sep=''),header=T,sep=",")
NSF_CS<-tmpNSF_CS[,2]

### Here we will load the NIH BIO scholar data 
tmpNIH_BIO<-read.csv(paste(data_path,"/Total/Scholars_total_BIO.csv", sep=''),header=T,sep=",")
NIH_BIO<-tmpNIH_BIO[,2]

## ATHLETES

### Here we will load the Olympic Medals Scores for USA athletes 
tmpOMS_USA<-read.csv(paste(data_path,"/Total/OMS_total_USA.csv", sep=''),header=T,sep=",")
OMS_USA<-tmpOMS_USA[,3]

### Here we will load the Olympic Medals Scores for FRA athletes 
tmpOMS_FRA<-read.csv(paste(data_path,"/Total/OMS_total_FRA.csv", sep=''),header=T,sep=",")
OMS_FRA<-tmpOMS_FRA[,3]

### Here we will load the Olympic Medals Scores for GBR athletes 
tmpOMS_GBR<-read.csv(paste(data_path,"/Total/OMS_total_GBR.csv", sep=''),header=T,sep=",")
...
setwd(paste(data_path, "/scripts_results", sep=""))
...
save(bsLN, bsLN_p, bsPL, bsPL_p, estln, estpl, LNest, PLest, bsLN_pall, bsLNall, bsPL_pall, bsPLall, m_ln, m_pl, file="results1.RData")

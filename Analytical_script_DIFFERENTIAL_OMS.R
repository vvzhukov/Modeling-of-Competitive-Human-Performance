...
data_path <- "/Users/apple/UH-CPL/ACEs/data_12122019"  ### This is for the MAC
output_sbufolder <- "/scripts_results/DIFFERENTIAL/"
setwd(data_path) ### will store figures at that folder

datasets <- c("OMS_FRA", "OMS_USA", "OMS_GBR")

tmpOMS_NOCL<-list(read.csv(paste(data_path,"/temporal_differential/OMS_differential_FRA_phases.csv", sep=""),header=T,sep=","),
               read.csv(paste(data_path,"/temporal_differential/OMS_differential_USA_phases.csv", sep=""),header=T,sep=","),
               read.csv(paste(data_path,"/temporal_differential/OMS_differential_GBR_phases.csv", sep=""),header=T,sep=","))
...
OMS_time<-c(1,2,3,4,5,6)		### 6=1912, 11=1936, 15=1960, 19=1976, 26=1998, 35=2000

OMS_NOC_1912<-tmpOMS_NOC[tmpOMS_NOC[,4]==CTV[OMS_time[1]],2]		### USA olympic scores in phase 1912
OMS_NOC_1936<-tmpOMS_NOC[tmpOMS_NOC[,4]==CTV[OMS_time[2]],2]		### USA olympic scores in phase 1936
OMS_NOC_1960<-tmpOMS_NOC[tmpOMS_NOC[,4]==CTV[OMS_time[3]],2]		### USA olympic scores in phase 1960
OMS_NOC_1976<-tmpOMS_NOC[tmpOMS_NOC[,4]==CTV[OMS_time[4]],2]		### USA olympic scores in phase 1976
OMS_NOC_1998<-tmpOMS_NOC[tmpOMS_NOC[,4]==CTV[OMS_time[5]],2]		### USA olympic scores in phase 1998
OMS_NOC_2016<-tmpOMS_NOC[tmpOMS_NOC[,4]==CTV[OMS_time[6]],2]		### USA olympic scores in phase 2016
...
setwd(paste(data_path, output_sbufolder, datasets[i], sep=""))
...
main_function=function(x){
    tryCatch(
        ...
        error=function(cond) {
            print(paste("Error caught! Additional data record used. Error text: ", cond))
            ...
            main_function(x) } ### Recursive call after dataset modification
    )
}
...
toc<-date()
tic
save(Data,bsLN, bsLN_p, bsPL, bsPL_p, estln, estpl, LNest, PLest, bsLN_pall, bsLNall, bsPL_pall, bsPLall, m_ln, m_pl, file="results2.RData")
toc
}
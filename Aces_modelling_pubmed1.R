...
for (i in 1:length(datasets)) {
    ...
    for (i in 1:n){
        ...
        PLest[i,6]<-estpl$gof
        bsPL <- bootstrap(m_pl, no_of_sims= BSiter, seed=1, threads=12)		### here we will get the sd values
        ...
        bsPL_p <- bootstrap_p(m_pl,no_of_sims= BSiter, seed=1, threads=12)				### here we will get the p-value regarding the PL fit
        ...
        LNest[i,8]<-estln$gof
        bsLN <- bootstrap(m_ln, no_of_sims= BSiter, seed=1, threads=12)		### here we will get the sd values
        ...
        bsLN_p <- bootstrap_p(m_ln,no_of_sims= BSiter, seed=1, threads=12)		### here we will get the p-value regarding the LN fit

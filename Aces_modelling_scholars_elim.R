...
SCH_full <- read.csv(paste("/Users/apple/Downloads/Data-5/Scholars/FULL_scholars_QC_temporal.csv", sep=''),
                     header = T)

BIO_full <- subset(SCH_full, dept == 'BIO')
BIO_I <- subset(BIO_full, y1996 != 0 | y1997 != 0 | y1998 != 0 | 
                    y1999 != 0 | y2000 != 0)
...
max_length <- max(length(gen1))

# trim top 20
results_max <- data.frame(gen1)

for (i in c(1:20))
    {
        temp <- results_max[,i][results_max[,i] < max(results_max[,i])]
        temp <- c(temp, rep(0,max_length - length(temp)))
        results_max <<- cbind(results_max, temp)
        colnames(results_max)[i+1] <- paste("gen1_max-", i)
    }

# trim bottom 20
results_min <- data.frame(gen1)

for (i in c(1:20))
{
    temp <- results_min[,i][results_min[,i] > min(results_min[,i][results_min[,i] > 0])]
    temp <- c(temp, rep(0,max_length - length(temp)))
    results_min <<- cbind(results_min, temp)
    colnames(results_min)[i+1] <- paste("gen1_min-", i)
}

datasets <- c(
              "SCHOLARS_BIO_I_GEN_top_elim",
              "SCHOLARS_BIO_I_GEN_bot_elim"
              )
...
VarNamesL<-list(
                colnames(results_max),
                colnames(results_min)
                )
